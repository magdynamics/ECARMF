    ...
    <div>
      ...
      {editing && (
        <section className="panel">
          ...
          {editing.linkedActionRef && (
            <p className="muted small">
              Linked remediation action: <span className="mono">{editing.linkedActionRef.slice(0, 8)}</span> · status {editing.status}
              {editing.residualSeverity != null && editing.residualLikelihood != null &&
                ` · residual ${editing.residualSeverity}×${editing.residualLikelihood} (from ${editing.inherentSeverity}×${editing.inherentLikelihood})`}
            </p>
          )}
        </section>
      )}
