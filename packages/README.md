...
| `ai-tenant10-rcm-v1.json` | ecarmf.ai-tenant10-rcm 1.0.0 | Tenant-10 medical-billing / RCM reference tenant (Regulated tier). The Risk Register is riskType-tagged KPIs whose multiplicative index (R16) is written inline as a KPI formula (verified live: a PHI assessment emits KPIActual 4800 tagged PHIBreach with a Critical deviation); departmental revenue-cycle KPIs are the child scores a Revenue Cycle `CompositeHealth` rolls up over `RollsUpInto` EntityRelationship edges (R13/R14, verified live: 0.9067); per-USER coder/biller accuracy KPIs (R7); HIPAA/OIG knowledge assets; compliance/revenue-cycle/executive-risk agents carrying the full Identity block. |
