...
/// manifest is stored as a JSON document alongside queryable identity columns.</summary>
public class EfPackageStore : IPackageStore
{
    ...
    public Task<bool> ExistsAsync(string packageId, string packageVersion, CancellationToken ct = default)
    {
        return _db.KnowledgePackages.AnyAsync(
            p => p.PackageId == packageId && p.PackageVersion == packageVersion, ct);
    }

    public async Task AddAsync(KnowledgePackageManifest manifest, PackageLoadState state, string? statusDetail, CancellationToken ct = default)
    {
    ...
    public async Task<StoredPackage?> GetAsync(string packageId, string packageVersion, CancellationToken ct = default)
    {
    ...
    public async Task UpdateStateAsync(string packageId, string packageVersion, PackageLoadState state, string? statusDetail, CancellationToken ct = default)
    {
        var record = await _db.KnowledgePackages.FirstOrDefaultAsync(
            p => p.PackageId == packageId && p.PackageVersion == packageVersion, ct)
            ?? throw new InvalidOperationException(
                $"Package '{packageId}' version '{packageVersion}' is not persisted.");
    ...
    public async Task<IReadOnlyList<StoredPackage>> GetAllAsync(CancellationToken ct = default)
    {
        var records = await _db.KnowledgePackages.AsNoTracking().ToListAsync(ct);
        return records.Select(ToStoredPackage).ToList();
    }

    public async Task<IReadOnlyList<StoredPackage>> GetByStateAsync(PackageLoadState state, CancellationToken ct = default)
    {
    ...
    private static StoredPackage ToStoredPackage(KnowledgePackageRecord record)
    {
        ...
        return new StoredPackage(manifest, state, record.StatusDetail);
    }
}
