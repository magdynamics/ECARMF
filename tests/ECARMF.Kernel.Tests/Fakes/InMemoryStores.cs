...
public class InMemoryTransactionStore : ITransactionStore
{
        ...
        TransactionQuery query, CancellationToken ct = default)
    {
        ...
        var filtered = Items
            ...
            .Where(t => string.IsNullOrWhiteSpace(query.UnitRef)
                || string.Equals(t.UnitRef, query.UnitRef, StringComparison.OrdinalIgnoreCase)
                || (!query.UnitExclusive && t.UnitRef is null))
...
public class InMemoryScoreStore : IScoreStore
{
    ...
    public Task<IReadOnlyList<ScoreRecord>> GetRecentRiskAllTenantsAsync(
        int limit, CancellationToken ct = default) =>
        Task.FromResult<IReadOnlyList<ScoreRecord>>(
            Items.Where(s => !string.IsNullOrEmpty(s.RiskType))
                .OrderByDescending(s => s.ComputedAt).Take(limit).ToList());
}
